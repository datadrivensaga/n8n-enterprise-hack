...
export class ProjectService {
	...
	/**
   * Cria um projeto de equipe se estiver dentro do limite de licença
   */
	async createTeamProject(adminUser: User, data: CreateProjectDto): Promise<Project> {
		...
		const project = this.projectRepository.create({ ...data, type: 'team' });
		await this.projectRepository.save(project);
	
		// Associa o usuário admin com tenantId correto
		await this.addUser(
		  project.id,
		  adminUser.id,
		  'project:admin',
	...
	/**
   * Busca ou cria o projeto pessoal do usuário
   */
	async getPersonalProject(user: User): Promise<Project> {
		// Verifica existência por tipo, tenant e relação
		...
		// Cria novo projeto pessoal
		const projectName = user.firstName && user.lastName
		...
		// Cria relação de ownership com tenant correto
		await this.addUser(
		  newProject.id,
		  user.id,
